...
export class TransactionHost<TAdapter = never> {
  ...
  constructor(
    ...
    this.transactionInstanceSymbol = getTransactionClsKey(this._options.connectionName);
  }
  ...
  get tx(): TTxFromAdapter<TAdapter> {
    ...
    // @ts-ignore
    return this.cls.get(this.transactionInstanceSymbol) ?? this._options.getFallbackInstance();
  }
   ...
   */
  withTransaction<R>(fn: (...args: any[]) => Effect.Effect<R>): Effect.Effect<R>;
  /**
   * Wrap a function call in a transaction defined by the adapter.
   *
   * The transaction instance will be accessible on the TransactionHost as `tx`.
   ...
   * @returns Whatever the passed function returns
   */
  withTransaction<R>(propagation: Propagation, fn: (...args: any[]) => Effect.Effect<R>): Effect.Effect<R>;
  /**
   * Wrap a function call in a transaction defined by the adapter.
   *
   * The transaction instance will be accessible on the TransactionHost as `tx`.
  ...
  withTransaction<R>(firstParam: any, secondParam?: any, thirdParam?: (...args: any[]) => Effect.Effect<R>) {
    let propagation: string;
    let options: any;
    let fn: (...args: any[]) => Effect.Effect<R>;
    if (thirdParam) {
  ...
  private decidePropagationAndRun(propagation: string, options: any, fn: (...args: any[]) => Effect.Effect<any>) {
    ...
    switch (propagation) {
      ...
      case Propagation.Never:
        ...
        return this.withoutTransaction(fn);
      default:
        throw new TransactionPropagationError(`Unknown propagation mode ${propagation}`);
    }
  }

  private runWithTransaction(options: any, fn: (...args: any[]) => Effect.Effect<any, any, any>) {
    return this.cls.run({ ifNested: 'inherit' }, () =>
      this._options
        .wrapWithTransaction(options, fn, this.setTxInstance.bind(this))
        .pipe(Effect.ensuring(Effect.sync(() => this.setTxInstance(undefined)))),
    );
  }
   ...
   * @returns Whatever the passed function returns
   */
  withoutTransaction<R>(fn: (...args: any[]) => Effect.Effect<R>): Effect.Effect<R> {
    return this.cls.run({ ifNested: 'inherit' }, () =>
      pipe(
        Effect.sync(() => this.setTxInstance(undefined)),
        fn,
        Effect.ensuring(Effect.sync(() => this.setTxInstance(undefined))),
      ),
    );
  }
  ...
  private setTxInstance(txInstance?: TTxFromAdapter<TAdapter>) {
    ...
    if (this._options.enableTransactionProxy) {
      this.cls.setProxy(getTransactionToken(this._options.connectionName), txInstance);
    }
  }
}
...
export function getTransactionHostToken(connectionName?: string) {
  return connectionName ? Symbol.for(`${TransactionHost.name}_${connectionName}`) : TransactionHost;
}
